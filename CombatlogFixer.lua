...
module.Init = function()
   ...
   SlashCmdList["LOGFIXER"] = function(msg)
      ...
      if( msg == "status" ) then
         ...
         for key, text in pairs(optionText) do
            if( CombatLogFixDB[key] ) then
               print(string.format("%s is |cff20ff20enabled|r", text))
            else
               print(string.format("%s is |cffff2020disabled|r", text))
            end
         end
         return
      end
      ...
      if( not optionText[msg] ) then
         ...
         print("/logfix zone - Toggles clearing on zone type change")
         print("/logfix auto - Toggles clearing combat log when it breaks")
         print("/logfix wait - Toggles not clearing until you drop combat")
         print("/logfix report - Toggles reporting how many messages were found when it broke")
         print("/logfix status - List of set options")
         return
      end
   ...
   -- Slash commands
   function Print(msg)
      print(string.format("|cff33ff99Log Fixer|r: %s", msg))
   end
