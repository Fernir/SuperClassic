...
module.Init = function()
	...
	local EXPIRING_DURATION = 1 --the minimum number of seconds a cooldown must be to use to display in the expiring format
	...
	local MINUTES_FORMAT = "|cffffffff%dм|r" --format for timers that have minutes remaining
	local HOURS_FORMAT = "|cff66ffff%dч|r" --format for timers that have hours remaining
	local DAYS_FORMAT = "|cff6666ff%dд|r" --format for timers that have days remaining
	...
	SecondsToTimeAbbrev = function(t)
		local hr, m, s, text
		if t <= 0 then text = ""
		elseif(t < 3600 and t > 60) then
			hr = floor(t / 3600)
			m = floor(mod(t, 3600) / 60 + 1)
			text = format("%dm", m)
		elseif t < 60 then
			m = floor(t / 60); s = mod(t, 60)
			if s > 1 then
				text = (m == 0 and format("%ds", s))
			else
				text = (m == 0 and format("|cffcc0000%.1fs|r", s))
			end
		else
			hr = floor(t / 3600 + 1)
			text = format("%dh", hr)
