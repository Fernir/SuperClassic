...
module.Init = function()
	...
	if SCDB[module.name]["Pivot to cursor"] == nil then SCDB[module.name]["Pivot to cursor"] = false end
	if SCDB[module.name]["Player titles"] == nil then SCDB[module.name]["Player titles"] = true end
	if SCDB[module.name]["Colorized status bar"] == nil then SCDB[module.name]["Colorized status bar"] = true end
	if SCDB[module.name]["Background color"] == nil then SCDB[module.name]["Background color"] = {.05, .05, .05, 1} end --Background color
	if SCDB[module.name]["Border color"] == nil then SCDB[module.name]["Border color"] = {.3, .3, .3, 1} end --Border color
	if SCDB[module.name]["Guild color"] == nil then SCDB[module.name]["Guild color"] = { 255/255, 20/255, 200/255, 1} end --Guild Color
	if SCDB[module.name]["Text target you"] == nil then SCDB[module.name]["Text target you"] = "<You>" end
	if SCDB[module.name]["World boss"] == nil then SCDB[module.name]["World boss"] = "??" end
	if SCDB[module.name]["Rare and elite mob"] == nil then SCDB[module.name]["Rare and elite mob"] = "Rare+" end
	if SCDB[module.name]["Rare mob"] == nil then SCDB[module.name]["Rare mob"] = "Rare" end
	...
	GameTooltipStatusBar:SetStatusBarTexture("Interface\\WorldStateFrame\\WorldState-CaptureBar")
   GameTooltipStatusBar:GetStatusBarTexture():SetTexCoord(0.8203125, 1.0, 0.34375, 0.484375)
	...
	bg:SetTexture("Interface\\WorldStateFrame\\WorldState-CaptureBar")
	bg:SetTexCoord(0.8203125, 1.0, 0.34375, 0.484375)
	...
	for i, v in ipairs(Tooltips) do
		v:SetBackdrop({ bgFile = "Interface\\Tooltips\\UI-Tooltip-Background", edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border", tileSize = 16, edgeSize = 16, insets = {left=3, right=3, top=3, bottom=3}, })
		v:SetBackdropColor(.01, .01, .01, .85)
		v:SetBackdropBorderColor(.4, .4, .4, 1)
